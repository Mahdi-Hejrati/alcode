...
class ${fd.name|filters.cap}_Controller_Base(Connection_Almas):
    ...
    def recievie(self, topic, value):
        ...
        ${'el' if not loop.first else ''}if topic == '${tp.topic.name}':
            self.on_${tp.topic.name}(${tp.topic.start_type.name|filters.cap}(json.loads(value)))
        % endfor
        else:
...
ALMAS-FILE-INFO
rabbitmq/python/${fd.name|filters.cap}/Core/${fd.name|filters.cap}_Controller_Base.py
ALMAS-FILE-SEPRATE
% endfor
